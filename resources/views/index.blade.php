...
<head>
    ...
    <style>
        ...
        *::after {
        ...
        .drop-zone:hover,
        .drop-zone.over { border-color: var(--accent); background: var(--primary-light); }
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card c-green">
                ...
                <div class="stat-info">
                    ...
                    <div class="stat-val">
                        {{ number_format(\App\Models\DataKunjungan::whereRaw("LOWER(jenis_kelamin) LIKE '%laki%'")->count()) }}
                    </div>
                </div>
            </div>
            <div class="stat-card c-sky">
                ...
                <div class="stat-info">
                    ...
                    <div class="stat-val">
                        {{ number_format(\App\Models\DataKunjungan::whereRaw("LOWER(jenis_kelamin) LIKE '%perempuan%' OR LOWER(jenis_kelamin) LIKE '%wanita%'")->count()) }}
                    </div>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if ($item->foto_ktp)
                                        <a href="{{ asset('storage/' . $item->foto_ktp) }}" target="_blank"
                                            style="color:var(--primary); font-weight:500; font-size:12px;">
                                            <i class="fas fa-id-card" style="margin-right:3px;"></i>Lihat KTP
                                        </a>
                                    @else
                                ...
                                <td>
                                    @if ($item->foto_diri)
                                        <a href="{{ asset('storage/' . $item->foto_diri) }}" target="_blank"
                                            style="color:var(--primary); font-weight:500; font-size:12px;">
                                            <i class="fas fa-portrait" style="margin-right:3px;"></i>Lihat Foto
                                        </a>
                                    @else
                                        <span style="color:var(--text-light);">–</span>
                                    @endif
                                ...
                                <td>
                                    <div class="action-wrap">
                                        {{-- Semua data item di-pass sebagai JSON ke openEdit --}}
                            ...
                            <tr>
                                <td colspan="13">
                                    <div class="empty-wrap">
                                        ...
                                        <p>Klik tombol <strong>Import Excel</strong> di pojok kanan atas untuk
                                            mengunggah data.</p>
                                    </div>
                                </td>
                            </tr>
    ...
    <script>
        ...
        function onPick(input) {
            ...
            if (!['xlsx', 'xls'].includes(ext)) {
                toast('err', 'Format Salah', 'Hanya file .xlsx atau .xls.');
                clearFile(); return;
            }
        ...
        function doImport() {
            ...
            const iv = setInterval(() => {
                if (p < 85) { p += Math.random() * 8; setProg(Math.min(p, 85)); }
            }, 300);
        ...
        function openEdit(id, wbp, peng, jk, hub, sub, alamat, nik, tgl, catatan, fotoKtp, fotoDiri) {
            ...
            setCurrentFoto('previewKtp', 'previewKtpWrap', 'currentKtpText', fotoKtp, 'KTP');
            setCurrentFoto('previewDiri', 'previewDiriWrap', 'currentDiriText', fotoDiri, 'Foto Diri');

            document.getElementById('modalEdit').classList.add('show');
        }

        function setCurrentFoto(imgId, wrapId, textId, path, label) {
        ...
        // ── FIX UTAMA: pakai FormData agar bisa kirim file foto ──────────────
        function doUpdate() {
            const id = document.getElementById('eId').value;

            // Gunakan FormData agar file (foto) bisa ikut dikirim
            const fd = new FormData();
            ...
            // Lampirkan foto hanya jika ada file baru dipilih
            ...
            // Kirim POST ke /kunjungan/{id} dengan _method=PUT (Laravel method spoofing)
            fetch(`/kunjungan/${id}`, {
                method: 'POST',
                headers: {
                    'X-CSRF-TOKEN': CSRF,
                    'Accept': 'application/json',
                    // JANGAN set Content-Type saat pakai FormData — biarkan browser yang set boundary
                },
                body: fd,
            })
            .then(r => r.json())
        ...
        function doDelete(id) {
            ...
            // FIX: URL harus /kunjungan/{id} sesuai route resource
            fetch(`/kunjungan/${id}`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': CSRF,
        ...
        function setBtnLoad(which, state) {
            ...
            document.getElementById('txt'  + which).textContent    = state
                ? 'Memproses...'
                : (which === 'Save' ? 'Simpan' : 'Simpan Perubahan');
